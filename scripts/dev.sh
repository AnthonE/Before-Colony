#!/usr/bin/env bash
# Build the browser client and run a local sector with Mobile Dolls and one AI agent.
#   scripts/dev.sh            then open http://127.0.0.1:8080 in Chrome or Edge
# Env: BC_DOLLS (default 24), BC_AGENTS (default 1), BC_ORACLE (local|jev; jev needs TYPESAFE_API_KEY)
set -euo pipefail
cd "$(dirname "$0")/.."
