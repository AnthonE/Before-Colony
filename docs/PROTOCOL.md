...
| roll (`i8`) | 8 |
| buttons: FIRE_PRIMARY, FIRE_SECONDARY, MELEE, BOOST, BRAKE, FLIGHT_ASSIST*, ZERO*, RCS_SHARP, GRAB*, STOW, THROW, JETTISON (*states) | 12 |
...
client is silent (the server repeats its last command, keeping only FLIGHT_ASSIST, ZERO and GRAB);
presses (STOW, THROW, JETTISON, MELEE) act on the tick they first appear.
...
| own (1 + 610 bits) | slot, generation, frame, alive, pos, vel (f32), rot (16-bit), ang_vel, propellant (f32), g_strain, heat, energy, ammo ×2, weapon_ready, charge, parts ×6, zero_strain, zero_mode, flags, ambac/thrust factors, respawn_in, extra mass (kg, i18), cargo ×4 (kg, 14 bits each), credits (24), held chunk (10) |
...
| 7 | extension | 3-bit sub-kind: 0 = RockBreak {id, rock, by}; 1–7 reserved |
